...
export interface AppConfig {
  ...
  /**
   * Пускать запросы к ИИ через системный прокси (VPN).
   *
   * По умолчанию НЕТ: мёртвый прокси от выключенного VPN ломает запросы к
   * сервисам, доступным и так. Но если VPN включён, без этой галочки он
   * приложению бесполезен — замер показал, что OpenRouter через прокси
   * отвечает, а мимо прокси отдаёт 403.
   */
  useProxy?: boolean
}
...
export interface AppData {
  ...
  /**
   * Когда состояние записано в последний раз.
   *
   * Нужно синхронизации: при слиянии двух устройств надо решить, чьи цели,
   * расписание и ключи брать, а по самим данным этого не видно.
   */
  savedAt?: string
}
