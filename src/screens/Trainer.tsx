  ...
  if (!attempts.length) {
    return (
      <div className="empty">
        <div className="big">📊</div>
        <p>
          Статистика появится, как только решишь первые задания. Быстрый путь —{' '}
          <a href="#/diagnostic">диагностический срез</a>: один заход по всем номерам, и приложение
          перестанет советовать вслепую.
        </p>
      </div>
    )
  }
          ...
          <div className="card" key={'prio_' + sid}>
            ...
            {list.map((p) => (
              <div className="row wrap" key={p.taskNo} style={{ gap: 10, padding: '8px 0', borderTop: '1px solid var(--line)' }}>
                ...
                <span className="small muted" style={{ width: 108, textAlign: 'right' }}>
                  ...
                  {p.rough && <span title="Попыток мало — цифра грубая"> ?</span>}
                </span>
            ...
            {blind.length > 0 && (
              <p className="small muted" style={{ marginTop: 10, marginBottom: 0 }}>
                Ещё не мерили: {blind.map((n) => '№' + n).join(', ')}. По ним счёт не ведётся, и
                список выше судит по неполной работе.{' '}
                <a href="#/diagnostic">Пройти диагностику</a> — один заход по всем номерам, и
                карта станет полной.
              </p>
            )}
          </div>
